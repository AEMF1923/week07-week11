...
/*
 * Does a JDBC call mean that we use things like executeQuery specifically? 
 * This brings a project information by project name according to the sql but does 
 * the name for this method make sense? 
 */
...
/* 
 * WE TRY TO CONNECT TO THE DATABASE FIRST THEN WE ADD THE PREPARED STATEMENT 
 */
		    try(Connection conn = DbConnection.getConnection()) {
		      startTransaction(conn);
...
/*
 * This brings back information 
 * WHye does this not have the start transaction bit 
 */
...
/*
 * The boolean return datatype is dictated by project service. That is where we built it 
 * This is the sql that is sent to the database and the you enter in the console all those question mark items listed below 
 * This is how you update a table row or entry via java 
 * I am not sure if I interpreting my error correctly
 */
	public boolean modifyProjectDetails(Project project) {
		...
		try(Connection conn = DbConnection.getConnection()){
			...
			try(PreparedStatement stmt = conn.prepareStatement(sql)) {
				...
				boolean modified = stmt.executeUpdate() == 1; //MODIFIED IS DEFINED HERE
				commitTransaction(conn); 
	...
	} //this marks the end of this method called modify project details 
	...
	public boolean deleteProject(Integer projectId) {
		...
		try(Connection conn = DbConnection.getConnection()){
			...
			throw new DbException(e);  //this catches the connection ...can we connect or not error
		}
	} //this marks the end of the method called delete project 
